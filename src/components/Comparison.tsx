...
export const Comparison: React.FC = () => {
  ...
  return (
    <section className="py-16 sm:py-24 border-b border-slate-200/80 relative bg-white">
      <div className="container px-4 md:px-8 max-w-6xl mx-auto">
        ...
        <div className="glass-card-cro rounded-3xl border border-slate-200 overflow-hidden shadow-xl bg-white">
          <div className="hidden sm:grid grid-cols-12 bg-slate-50 border-b border-slate-200 p-5 text-xs font-extrabold font-space tracking-wider uppercase text-slate-600">
            <div className="col-span-4">RECURSO / PROCESSO</div>
            <div className="col-span-4 text-red-600 flex items-center gap-1.5 font-bold">
              <XCircle className="h-4 w-4 text-red-500" />
              <span>SEM O LOJAPOD</span>
            </div>
            <div className="col-span-4 text-emerald-700 flex items-center gap-1.5 font-bold">
            ...
            {comparisonData.map((item, idx) => (
              <div key={idx} className="p-4 sm:p-5 hover:bg-slate-50/70 transition-colors">
                ...
                <div className="hidden sm:grid grid-cols-12 text-sm items-center">
                  <div className="col-span-4 font-bold font-space text-slate-900">
                    {item.feature}
                  </div>
                  <div className="col-span-4 text-slate-600 text-xs sm:text-sm flex items-start gap-2">
                    <XCircle className="h-4 w-4 text-red-500 shrink-0 mt-0.5" />
                    <span>{item.without}</span>
                  </div>
                  <div className="col-span-4 text-emerald-800 text-xs sm:text-sm font-semibold flex items-start gap-2">
                ...
                <div className="sm:hidden space-y-2.5">
                  <div className="font-extrabold font-space text-slate-900 text-sm border-b border-slate-100 pb-1.5 flex justify-between items-center">
                    <span>{item.feature}</span>
                    <span className="text-[10px] text-slate-400 font-mono">#0{idx + 1}</span>
                  </div>
                  <div className="grid grid-cols-1 gap-2 text-xs">
                    <div className="p-2.5 rounded-xl bg-red-50/70 border border-red-200 text-slate-700 flex items-start gap-2">
                      <XCircle className="h-4 w-4 text-red-500 shrink-0 mt-0.5" />
                      <div>
                        <div className="text-[10px] text-red-600 font-bold uppercase">Sem LojaPod:</div>
                        <div>{item.without}</div>
                      </div>
                    </div>
                    <div className="p-2.5 rounded-xl bg-emerald-50/80 border border-emerald-200 text-emerald-900 font-medium flex items-start gap-2">
                      <CheckCircle2 className="h-4 w-4 text-emerald-600 shrink-0 mt-0.5" />
                      <div>
                        <div className="text-[10px] text-emerald-700 font-extrabold uppercase">Com LojaPod Pro:</div>
                        <div>{item.with}</div>
                      </div>
                    </div>
                  </div>
